...
def send_request():
    #爬取7页，可自行修改
    for link in ip_pool_list:
        print(link.format(1))
        for i in range(1,8): 
            print(f'正在抓取第{i}页……')
            # response = requests.get(url=f'http://www.ip3366.net/free/?page={i}&stype=2', headers=request_header())
            response = requests.get(url=link.format(i), headers=request_header())
...
def test_ip(proxy):
    ...
    try:
        response = requests.get(url=test_url,headers=request_header(),proxies=proxies,timeout=1) #设置timeout，使响应等待1s
        response.close()
